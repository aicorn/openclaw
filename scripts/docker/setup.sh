...
ensure_control_ui_allowed_origins() {
  ...
  current_allowed_origins="$(
    "${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm openclaw-cli \
...
if docker compose version >/dev/null 2>&1; then
  ...
  echo "Docker Compose not available (try: docker compose version  or  docker-compose version)" >&2
  exit 1
fi
...
"${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm --user root --entrypoint sh openclaw-cli -c \
...
echo ""
"${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm openclaw-cli onboard --mode local --no-install-daemon
...
"${DC[@]}" "${COMPOSE_ARGS[@]}" up -d openclaw-gateway
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if ! "${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker openclaw-gateway --version >/dev/null 2>&1; then
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if ! "${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm --no-deps openclaw-cli \
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    if ! "${DC[@]}" "${BASE_COMPOSE_ARGS[@]}" run --rm --no-deps openclaw-cli \
    ...
    "${DC[@]}" "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate openclaw-gateway
  fi
else
  ...
  # config alone.
  if ! "${DC[@]}" "${COMPOSE_ARGS[@]}" run --rm openclaw-cli \
    config set agents.defaults.sandbox.mode "off" >/dev/null; then
